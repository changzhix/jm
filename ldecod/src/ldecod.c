...
void Configure(int ac, char *av[])
{
  ...
  params->poc_scale=2;
  params->silent = FALSE;
  params->disable_postprocessing = FALSE;
  ...
  if (ac==2)
  {
    ...
    else if (0 == strncmp (av[1], "-d", 2))
    {
      params->disable_postprocessing = TRUE;
    }
    else
    {
  ...
  while (CLcount < ac)
  {
    ...
    if (0 == strncmp (av[CLcount], "-d", 2))  // Disable postprocessing
    {
      params->disable_postprocessing = TRUE;
      CLcount ++;
    }
